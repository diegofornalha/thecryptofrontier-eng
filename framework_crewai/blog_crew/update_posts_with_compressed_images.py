    ...
    try:
        ...
        while quality > 20:
            ...
            if size_mb <= max_size_mb:
                logger.info(f"Imagem comprimida: {size_mb:.2f}MB (qualidade: {quality})")
                buffer.seek(0)
                return buffer.read()
...
def upload_image_to_sanity(image_path: Path) -> str:
    """Upload de imagem para o Sanity com compressão"""
    try:
        # Comprimir imagem
        ...
        # Upload com imagem comprimida
        files = {'file': (f"{image_path.stem}.jpg", compressed_image, 'image/jpeg')}
        headers = {'Authorization': f'Bearer {SANITY_API_TOKEN}'}
...
def main():
    logger.info("""
╔══════════════════════════════════════════════════════════════╗
║       ATUALIZAÇÃO DE POSTS COM IMAGENS COMPRIMIDAS          ║
╚══════════════════════════════════════════════════════════════╝
    """)
